...
// for CSS handling
    ...
    function _mergeFullCSS($p, &$t, $tag, $classes, $id)
    {
        ...
        // STYLESHEET CLASS e.g. #smallone{}  #redletter{}
        if (array_key_exists('ID>>' . $id, $p) && isset($id) && $id) {
            $this->_mergeCSS($p['ID>>' . $id], $t);
        }
        // STYLESHEET CLASS e.g. .smallone{}  .redletter{}
        foreach ($classes AS $class) {
            if (array_key_exists($tag . '>>CLASS>>' . $class, $p)) {
                $this->_mergeCSS($p[$tag . '>>CLASS>>' . $class], $t);
            }
        }
        // STYLESHEET CLASS e.g. #smallone{}  #redletter{}
        if (isset($id) && is_array($p)) {
            if (array_key_exists($tag . '>>ID>>' . $id, $p)) {
